...
void ClientHandler::newRequest()
{
    ...
    std::unique_ptr<Request> request{std::make_unique<Request>(QJsonDocument::fromJson(_socket->readAll()).object(), _dbManager)};

    // Передача управления запросом requestHandler
    RequestHandler* requestHandler = new RequestHandler(std::move(request));
...
{
    QByteArray jsonData{response.toJson().toJson()};
